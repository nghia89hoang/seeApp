...
uniform sampler2D uTex0; //texure to be applied to billboard quad
...
void main(void) {
    ...
    vec2 pixelRegionCoords = fract(TexCoord0.st/texCoordsStep); //x and y coordinates within "pixel region"
    vec2 pixelBin = floor(TexCoord0.st/texCoordsStep); //"pixel region" number counting away from base case
    ...
    avgColor = texture2D(uTex0, texCoords[0]) +
    texture2D(uTex0, texCoords[1]) +
    texture2D(uTex0, texCoords[2]) +
    texture2D(uTex0, texCoords[3]) +
    texture2D(uTex0, texCoords[4]) +
    texture2D(uTex0, texCoords[5]) +
    texture2D(uTex0, texCoords[6]) +
    texture2D(uTex0, texCoords[7]) +
    texture2D(uTex0, texCoords[8]);
